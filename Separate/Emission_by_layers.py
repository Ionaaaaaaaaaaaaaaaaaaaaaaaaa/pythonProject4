...
def Get_defects_loop(dir,coordTL, Ftime, r, defSizename, defectsName, step, step_save,defectsdictGlobal, defsizedictGlobal,lower_border,Higher_border):
    ...
    while time <= Ftime:
        ...
        while time <= Ftime:
            ...
            if round(time, 9) >= (round(time1, 9) + step_save):
                ...
                defectsdictLayer.update({time1: n})
                defsizedictLayer.update({time1: m})
                break
        continue
    defectsdictGlobal.update({str(lower_border) + '_' + str(Higher_border): defectsdictLayer})
    defsizedictGlobal.update({str(lower_border) + '_' + str(Higher_border): defsizedictLayer})
    return defectsdictGlobal, defsizedictGlobal

def Get_on_Layers(coordT, Ftime, r, defSizename, defectsName, step, step_save,dir,LayerThickness,Layerstep,lower_border,Top):
    Higher_border = lower_border + LayerThickness
    defectsdictGlobal = {}
    defsizedictGlobal = {}
    while Higher_border <= Top:
        coordTL = coordT[(coordT[4] <= Higher_border) & (coordT[4] >= lower_border)]
        print(coordTL)
        defectsdictLayer = {}
        defsizedictLayer = {}
        print('Layer------------------------>', lower_border, '=>', Higher_border)
        defectsdictGlobal, defsizedictGlobal = Get_defects_loop(dir,coordT, Ftime, r, defSizename, defectsName, step,
                                                                step_save,defectsdictGlobal, defsizedictGlobal,lower_border,Higher_border)
        lower_border += Layerstep
        Higher_border = lower_border + LayerThickness
    pickle.dump(defectsdictGlobal, open(dir + 'defectsdict.p', 'wb'))
    pickle.dump(defsizedictGlobal, open(dir + 'defsizedict.p', 'wb'))
def start():
    ...
    lower_border = float(input('Please enter the lower border of the object '))
    Top = float(input('Please enter the highest point of the object '))
    Get_on_Layers(coordT, Ftime, r, defSizename, defectsName, step, step_save,dir,LayerThickness,Layerstep,lower_border,Top)
    print('end')
    input('Press any key ')
