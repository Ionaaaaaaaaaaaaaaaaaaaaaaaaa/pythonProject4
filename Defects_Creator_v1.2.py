...
if Choice1 == 'new':
    ...
    def brokenbondstimes(data, bonddf,coordTcreate):
        b = {}
        used = np.array([])
        ...
        print(df)
        df.to_csv(dir + coordTcreate + ".csv")
        dirL = dir
        return b, df,coordTcreate,dirL


    partdf = parsingPart(datapart)
    bonddf = coordInfodf(data, partdf)
    b,TBonddf,coordTcreate,dirL = brokenbondstimes(data, bonddf,coordTcreate)
